...
public class ForgeMenu extends AbstractContainerMenu {
    ...
    public ForgeMenu(int pContainerId, Inventory inv, BlockEntity entity, ContainerData data) {
        ...
        // Fuel slot first (slot 0)
        this.addSlot(new SlotItemHandler(blockEntity.itemHandler, 0, 80, 54));

        // Top row - 9 slots (1–9)
        for (int col = 0; col < 9; col++) {
            this.addSlot(new SingleItemSlot(blockEntity.itemHandler, col + 1, 8 + col * 18, 18));
        }

        // Middle row - 6 slots (10–15)
        for (int col = 0; col < 3; col++) {
            this.addSlot(new SingleItemSlot(blockEntity.itemHandler, 10 + col, 8 + col * 18, 36));
        }
        for (int col = 3; col < 6; col++) {
            this.addSlot(new SingleItemSlot(blockEntity.itemHandler, 10 + col, 62 + col * 18, 36));
        }

        // Bottom row - 6 slots (16–21)
        for (int col = 0; col < 3; col++) {
            this.addSlot(new SingleItemSlot(blockEntity.itemHandler, 16 + col, 8 + col * 18, 54));
        }
        for (int col = 3; col < 6; col++) {
            this.addSlot(new SingleItemSlot(blockEntity.itemHandler, 16 + col, 62 + col * 18, 54));
        }
    ...
    public ItemStack quickMoveStack(Player playerIn, int pIndex) {
        ...
        boolean fromPlayer = pIndex < VANILLA_FIRST_SLOT_INDEX + VANILLA_SLOT_COUNT;
        boolean fromForge = pIndex >= TE_INVENTORY_FIRST_SLOT_INDEX && pIndex < TE_INVENTORY_FIRST_SLOT_INDEX + TE_INVENTORY_SLOT_COUNT;

        if (fromPlayer) {
            // Shift-click from player inventory
            if (ForgeBlockEntity.isFuel(sourceStack)) {
                // Send fuel to slot 0
                if (!moveItemStackTo(sourceStack, TE_INVENTORY_FIRST_SLOT_INDEX, TE_INVENTORY_FIRST_SLOT_INDEX + 1, false)) {
                    return ItemStack.EMPTY;
                }
            } else {
                // Send non-fuel to forge slots 1–21
            ...
            // Shift-click from forge inventory back to player inventory
